...
class DependencyGraph(object):
    ...
    def _trace_computational_graph(self, module2node, grad_fn_root, gradfn2module, reused):
        ...
        for (param, dim) in self.unwrapped_parameters:
            if param in module2node:
                ...
                if self.verbose:
                    warnings.warn(
                        f"Skipping unwrapped parameter not found in traced graph: "
                        f"{self._param_to_name.get(param, 'unknown')}"
                    )
        return module2node
    ...
    def _update_expand_index_mapping(self, node: Node):
        ...
        #             break
        """
        Update index mapping for GQA/MQA-style KV expansion.

        Works for Hugging Face Gemma/LLaMA-style repeat_kv:
            hidden_states: (batch, num_kv_heads, seqlen, head_dim)
            hidden_states[:, :, None, :, :].expand(batch, num_kv_heads, n_rep, seqlen, head_dim)
            -> reshape(batch, num_attention_heads, seqlen, head_dim)

        The old version was already conceptually right, but this one is safer for
        Gemma 3 / newer PyTorch because it:
        1) accepts both _saved_self_sym_sizes and _saved_self_sizes
        2) computes repeat from the saved input shape more defensively
        3) handles SymInt values by converting to int when possible
        """
        out_channels = None
        for n in node.outputs:
            out_channels = self._infer_in_channels_recursively(n)
        ...
        saved_sizes = None
        if hasattr(node.grad_fn, "_saved_self_sym_sizes"):
            saved_sizes = node.grad_fn._saved_self_sym_sizes
        elif hasattr(node.grad_fn, "_saved_self_sizes"):
            saved_sizes = node.grad_fn._saved_self_sizes

        if saved_sizes is None:
            return

        try:
            saved_sizes = [int(x) for x in saved_sizes]
        except Exception:
            return

        # Expected expand input for repeat_kv:
        # (batch, num_kv_heads, 1, seqlen, head_dim)
        if len(saved_sizes) != 5:
            return

        batch, num_key_value_heads, expand_dim, slen, head_dim = saved_sizes

        if expand_dim <= 0:
            return

        in_channels = num_key_value_heads * expand_dim * head_dim
        if out_channels is None or in_channels is None or in_channels == 0:
            return
        ...
        for out_node in node.outputs:
            for dep in node.dependencies:
                if any((dep is d) for d in addressed_dep):
                    continue
                if dep.target == out_node:
                    dep.index_mapping[0] = _helpers._ExpandIndexMapping(
                        repeat=repeat, reverse=False
                    )
                    addressed_dep.append(dep)
                    break
